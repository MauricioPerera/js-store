...
description: 'ensureIndex(field) mantiene en RAM un índice valor->ids; find usa el índice para igualdad simple sobre un campo indexado (evita escanear), y cae a escaneo para filtros complejos.'
...
- El resultado de `find` es **idéntico** con o sin índice (el índice solo cambia el CÓMO, no el QUÉ).
- `count`/`remove` pueden seguir escaneando (no es obligatorio optimizarlos); solo `find` usa el índice.
